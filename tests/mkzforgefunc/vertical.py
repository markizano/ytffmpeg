...
Environment Variables:
    MKZFORGE_CMD: Path to the mkzforge command to use (defaults to 'mkzforge')
    Set to '.venv/bin/mkzforge' to use virtual environment version
'''
...
# Get mkzforge command from environment or use default
# Convert to absolute path if it's a relative path to handle directory changes during tests
_mkzforge_cmd = os.environ.get('MKZFORGE_CMD', 'mkzforge')
if not os.path.isabs(_mkzforge_cmd) and os.path.exists(_mkzforge_cmd):
    MKZFORGE_CMD = os.path.abspath(_mkzforge_cmd)
else:
    MKZFORGE_CMD = _mkzforge_cmd
...
class TestVerticalVideoWorkflow(unittest.TestCase):
    ...
    def _test_create_new_project(self):
        """Test creating a new mkzforge project."""
        log.info("Testing project creation...")

        # Run mkzforge new command
        result = subprocess.run([
            MKZFORGE_CMD, 'new', str(self.project_dir)
        ...
        self.assertTrue((self.project_dir / 'mkzforge.yml').exists(), "Configuration file was not created")
        self.assertTrue((self.project_dir / 'readme.md').exists(), "README file was not created")

        # Verify initial configuration
        config = yaml.safe_load(open(self.project_dir / 'mkzforge.yml'))
    ...
    def _test_refresh_and_verify(self):
        """Test running mkzforge refresh and verify processing."""
        log.info("Testing refresh operation...")

        # Change to project directory
        os.chdir(str(self.project_dir))

        # Run mkzforge refresh with CPU and smaller model to avoid memory issues
        env = os.environ.copy()
        env.update({
        ...
        result = subprocess.run([
            MKZFORGE_CMD, 'refresh', '--device', 'cpu'
    ...
    def _test_build_and_verify_output(self):
        ...
        # Run mkzforge build
        result = subprocess.run([
            MKZFORGE_CMD, 'build', '--no-autoplay'
        ], capture_output=True, text=True)
